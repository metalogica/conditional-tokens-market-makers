  ...
  function ([, creator, oracle, investor1, trader, investor2]) {
    ...
    step("can be created by factory", async function () {
      ...
      const fixedProductMarketMakerAddress =
        await fixedProductMarketMakerFactory.createFixedProductMarketMaker.call(
          ...createArgs
        );
      const createTx =
        await fixedProductMarketMakerFactory.createFixedProductMarketMaker(
          ...createArgs
        );
    ...
    step("can be funded", async function () {
      ...
      const fundingTx = await fixedProductMarketMaker.addFunding(
        addedFunds1,
        initialDistribution,
        { from: investor1 }
      );
      ...
      (await collateralToken.balanceOf(investor1)).should.be.a.bignumber.equal(
        "0"
      );
      (
        await fixedProductMarketMaker.balanceOf(investor1)
      ).should.be.a.bignumber.equal(addedFunds1);
      ...
      for (let i = 0; i < positionIds.length; i++) {
        ...
        (
          ...
          addedFunds1.sub(expectedFundedAmounts[i])
        );
      }
    });
    ...
    step("can buy tokens from it", async function () {
      ...
      await fixedProductMarketMaker.buy(
        investmentAmount,
        buyOutcomeIndex,
        outcomeTokensToBuy,
        { from: trader }
    ...
    step("can sell tokens to it", async function () {
      ...
      await fixedProductMarketMaker.sell(
        returnAmount,
        sellOutcomeIndex,
        outcomeTokensToSell,
        { from: trader }
    ...
    step("can continue being funded", async function () {
      ...
      await fixedProductMarketMaker.addFunding(addedFunds2, [], {
        from: investor2,
      });
      ...
      for (let i = 0; i < positionIds.length; i++) {
        ...
        newMarketMakerBalance.should.be.a.bignumber
          ...
          .lte(marketMakerPool[i].add(addedFunds2));
        marketMakerPool[i] = newMarketMakerBalance;
    ...
    step("can be defunded", async function () {
      await fixedProductMarketMaker.removeFunding(burnedShares1, {
        from: investor1,
      });

      (await collateralToken.balanceOf(investor1)).should.be.a.bignumber.gt(
        "0"
      );
      (
        await fixedProductMarketMaker.balanceOf(investor1)
      ).should.be.a.bignumber.equal(addedFunds1.sub(burnedShares1));
