...
contract(
  ...
  function ([, creator, oracle, trader, investor2, testInvestor]) {
    ...
    step("can buy tokens from it", async function () {
      ...
      ).should.be.a.bignumber.equal(feePoolManipulationAmount.add(feeAmount));
    ...
    step("cannot leech wei by adding and removing funding", async function () {
      ...
      collateralBalanceBefore.should.be.a.bignumber.equal(
    ...
    step(
      "cannot raise fee pool ratio by removing funding down to 1 wei",
      async function () {
        const manipulationAmount = initialFunds.subn(1);
        const collateralBalanceBefore = await collateralToken.balanceOf(
          fixedProductMarketMaker.address
        );
        const collectedFeesBefore =
          await fixedProductMarketMaker.collectedFees();

        await fixedProductMarketMaker.removeFunding(manipulationAmount, {
          from: creator,
        });
        await collateralToken.deposit({
          from: creator,
        ...
        await fixedProductMarketMaker.addFunding(manipulationAmount, [], {
          from: creator,
        });
        ...
        postManipulationCreatorPoolShares =
          await fixedProductMarketMaker.balanceOf(creator);
      }
    );
    ...
    step("can sell tokens to it", async function () {
      ...
      fpmmCollateralBalanceAfter
        .sub(fpmmCollateralBalanceBefore)
        .should.be.a.bignumber.equal(feeAmount);
    ...
    step("can be defunded", async function () {
      ...
      (
        ...
        postManipulationCreatorPoolShares.sub(burnedShares1)
      );
