...
const { getConditionId, getCollectionId, combineCollectionIds, getPositionId } =
...
contract("Multi-condition", function (accounts) {
  ...
  before(async () => {
    ...
    await collateralToken.approve(lmsrFactory.address, funding, {
      from: accounts[investor],
    });

    lmsrInstance = await getParamFromTxEvent(
      await lmsrFactory.createLMSRMarketMaker(
        pmSystem.address,
    ...
    positionId1 = getPositionId(
      collateralToken.address,
      combineCollectionIds([c1o1CollectionId, c2o1CollectionId])
    );
    positionId2 = getPositionId(
      collateralToken.address,
      combineCollectionIds([c1o2CollectionId, c2o1CollectionId])
    );
    positionId3 = getPositionId(
      collateralToken.address,
      combineCollectionIds([c1o1CollectionId, c2o2CollectionId])
    );
    positionId4 = getPositionId(
      collateralToken.address,
      combineCollectionIds([c1o2CollectionId, c2o2CollectionId])
    );
  });
  ...
  it("Should have conditions in the system with the listed ConditionIDs", async () => {
    ...
    var numeratorLen = await pmSystem
      .payoutNumerators(conditionOneId, 0)
      .then((r) => r.toString());
    assert.equal(numeratorLen, 0);

    var numeratorLen2 = await pmSystem
      .payoutNumerators(conditionTwoId, 0)
      .then((r) => r.toString());
  ...
  it("The LMSR should have the correct amount of tokens at the specified positions", async () => {
    assert(
      funding.eq(await pmSystem.balanceOf(lmsrInstance.address, positionId1))
    );
    assert(
      funding.eq(await pmSystem.balanceOf(lmsrInstance.address, positionId2))
    );
    assert(
      funding.eq(await pmSystem.balanceOf(lmsrInstance.address, positionId3))
    );
    assert(
      funding.eq(await pmSystem.balanceOf(lmsrInstance.address, positionId4))
    );
  });
  ...
  it("Users should be able to buy a position", async () => {
    ...
    await collateralToken.approve(lmsrInstance.address, toBN(1e18), {
      from: accounts[trader],
    });
  ...
  it("Users should be able to make complex buy / sell orders", async () => {
    await pmSystem.setApprovalForAll(lmsrInstance.address, true, {
      from: accounts[trader],
    });
    await lmsrInstance.trade([-1e9, 0, -1e9, 0], toBN(1e18), {
      from: accounts[trader],
    });
