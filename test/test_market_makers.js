...
const { getCollectionId, getPositionId } =
...
contract("MarketMaker", function (accounts) {
  ...
  it("should move price of an outcome to 0 after participants sell lots of that outcome to lmsrMarketMaker maker", async () => {
    ...
    const questionId =
      "0xf00dcafef00dcafef00dcafef00dcafef00dcafef00dcafef00dcafef00dcafe";
    ...
    await etherToken.approve(lmsrMarketMakerFactory.address, funding, {
      from: accounts[investor],
    });
    assert.equal(
      await etherToken.balanceOf
        .call(accounts[investor])
        .then((v) => v.toString()),
      funding.toString()
    );
    ...
    await etherToken.deposit({
      value: tokenCount.mul(loopCount),
      from: accounts[trader],
    });
    await etherToken.approve(pmSystem.address, tokenCount.mul(loopCount), {
      from: accounts[trader],
    });
    await pmSystem.splitPosition(
      etherToken.address,
      "0x00",
      conditionId,
      [...Array(numOutcomes).keys()].map((i) => 1 << i),
      tokenCount.mul(loopCount),
      { from: accounts[trader] }
    );
    ...
    for (const i of _.range(loopCount.toNumber())) {
      ...
      outcomeTokenAmounts = Array.from({ length: numOutcomes }, (v, i) =>
        i === outcome ? tokenCount.neg() : toBN(0)
      );
      profit = (
        await lmsrMarketMaker.calcNetCost.call(outcomeTokenAmounts)
      ).neg();
      if (profit.eqn(0)) break;

      // Selling tokens
      assert.equal(
        (
          await getParamFromTxEvent(
            await lmsrMarketMaker.trade(outcomeTokenAmounts, profit.neg(), {
              from: accounts[trader],
            }),
            "outcomeTokenNetCost"
          )
      ...
      const expected = lmsrMarginalPrice(
        funding,
        netOutcomeTokensSold,
        outcome
      );
      const actual = new Decimal(
        await lmsrMarketMaker.calcMarginalPrice
          .call(toBN(outcome))
          .then((v) => v.toString())
      ).div(ONE);
      assert(
    ...
    assert(
      (await etherToken.balanceOf.call(accounts[trader])).gt(buyerBalance),
      "trader balance did not increase"
    );
  });
  ...
  it("should move price of an outcome to 1 after participants buy lots of that outcome from lmsrMarketMaker maker", async () => {
    ...
    for (const [investor, funding, tokenCountRaw] of [
      ...
      const loopCount = 10;
      await etherToken.deposit({
        value: tokenCount.muln(loopCount),
        from: accounts[trader],
      });
      ...
      for (const i of _.range(loopCount)) {
        ...
        outcomeTokenAmounts = Array.from({ length: numOutcomes }, (v, i) =>
          i === outcome ? tokenCount : toBN(0)
        );
        cost = await lmsrMarketMaker.calcNetCost.call(outcomeTokenAmounts);

        // Buying tokens
        await etherToken.approve(lmsrMarketMaker.address, cost, {
          from: accounts[trader],
        });
        assert.equal(
          await getParamFromTxEvent(
            await lmsrMarketMaker.trade(outcomeTokenAmounts, cost, {
              from: accounts[trader],
            }),
            "outcomeTokenNetCost"
          ).then((v) => v.toString()),
  ...
  it("should allow buying and selling outcome tokens in the same transaction", async () => {
    ...
    // User buys all outcomes
    await etherToken.deposit({
      value: initialOutcomeTokenCount.add(initialWETH9Count),
      from: accounts[trader],
    });
    await etherToken.approve(pmSystem.address, initialOutcomeTokenCount, {
      from: accounts[trader],
    });
    await pmSystem.splitPosition(
      etherToken.address,
      "0x00",
      conditionId,
      [...Array(numOutcomes).keys()].map((i) => toBN(1 << i)),
    ...
    assert.equal(
      await getParamFromTxEvent(
        await lmsrMarketMaker.trade(tradeValues, cost, {
          from: accounts[trader],
        }),
        "outcomeTokenNetCost"
      ).then((v) => v.toString()),
    ...
    for (const [tradeValue, i] of tradeValues.map((v, i) => [v, i])) {
      assert.equal(
        await pmSystem.balanceOf
          ...
          .then((v) => v.toString()),
        initialOutcomeTokenCount.add(tradeValue)
      );
    }

    assert.equal(
      await etherToken.balanceOf
        .call(accounts[trader])
        .then((v) => v.toString()),
      initialWETH9Count.sub(cost).toString()
    );
  });
});
...
contract("LMSRMarketMaker", function (accounts) {
  ...
  it("can be created and closed", async () => {
    ...
    await assertRejects(
      lmsrMarketMaker.close({ from: accounts[buyer] }),
      "lmsrMarketMaker closed twice"
    );
  ...
  it("should allow buying and selling", async () => {
    ...
    const positionId = getPositionId(
      etherToken.address,
      getCollectionId(conditionId, 1 << outcome)
    );
    const tokenCount = toBN(1e15);
    let outcomeTokenAmounts = Array.from({ length: numOutcomes }, (v, i) =>
      i === outcome ? tokenCount : toBN(0)
    );
    const outcomeTokenCost = await lmsrMarketMaker.calcNetCost.call(
      outcomeTokenAmounts
    );
    ...
    assert.equal(
      await pmSystem.balanceOf
        .call(accounts[buyer], positionId)
        .then((v) => v.toString()),
      tokenCount.toString()
    );
    assert.equal(
    ...
    assert.equal(
      await getParamFromTxEvent(
        ...
        "outcomeTokenNetCost"
      ).then((v) => v.neg().toString()),
      outcomeTokenProfit.toString()
    );
  ...
  it("should allow short selling", async () => {
    ...
    const differentPositionId = getPositionId(
      etherToken.address,
      getCollectionId(conditionId, 1 << differentOutcome)
    );
    const tokenCount = toBN(1e15);
    const outcomeTokenAmounts = Array.from({ length: numOutcomes }, (v, i) =>
      i !== outcome ? tokenCount : toBN(0)
    );
    const outcomeTokenCost = await lmsrMarketMaker.calcNetCost.call(
      outcomeTokenAmounts
    ...
    assert.equal(
      await pmSystem.balanceOf
        .call(accounts[buyer], differentPositionId)
        .then((v) => v.toString()),
      tokenCount.toString()
    );
  });
  ...
  it("trading stress testing", async () => {
    ...
    const positionIds = [...Array(numOutcomes).keys()].map((i) =>
      getPositionId(etherToken.address, getCollectionId(conditionId, 1 << i))
    );

    // Get ready for trading
    const tradingStipend = toBN(1e19);
    await etherToken.deposit({
      value: tradingStipend.muln(2),
      from: accounts[trader],
    });
    await etherToken.approve(pmSystem.address, tradingStipend, {
      from: accounts[trader],
    });
    await pmSystem.splitPosition(
      etherToken.address,
    ...
    // Allow all trading
    await etherToken.approve(lmsrMarketMaker.address, MAX_VALUE, {
      from: accounts[trader],
    });
    await pmSystem.setApprovalForAll(lmsrMarketMaker.address, true, {
      from: accounts[trader],
    ...
    for (let i = 0; i < 10; i++) {
      const outcomeTokenAmounts = randnums(-1e16, 1e16, numOutcomes).map((n) =>
        toBN(n.valueOf())
      );
      const netCost = await lmsrMarketMaker.calcNetCost.call(
        outcomeTokenAmounts
      );

      const lmsrMarketMakerOutcomeTokenCounts = await Promise.all(
        positionIds.map((positionId) =>
          pmSystem.balanceOf.call(lmsrMarketMaker.address, positionId)
        )
      );

      const lmsrMarketMakerCollateralTokenCount =
        await etherToken.balanceOf.call(lmsrMarketMaker.address);

      let txResult;
      try {
        txResult = await lmsrMarketMaker.trade(outcomeTokenAmounts, netCost, {
          from: accounts[trader],
        });
      } catch (e) {
        throw new Error(
          `trade ${i} with input ${outcomeTokenAmounts} and limit ${netCost} failed while lmsrMarketMaker has:\n\n${lmsrMarketMakerOutcomeTokenCounts
            .map((c) => c.toString())
            .join(
              "\n"
            )}\n\nand ${lmsrMarketMakerCollateralTokenCount.toString()}: ${
            e.message
          }`
        );
      }
      ...
      if (txResult)
        assert.equal(
          (
            await getParamFromTxEvent(txResult, "outcomeTokenNetCost")
          ).toString(),
          netCost.toString()
        );
    }
