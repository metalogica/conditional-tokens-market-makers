...
contract("LMSR gas costs", function ([lmsrOwner, oracle, trader]) {
  ...
  before(async function () {
    ...
    await collateralToken.deposit({
      value: totalCollateralAvailable,
      from: lmsrOwner,
    });
    await collateralToken.approve(lmsrMarketMakerFactory.address, MAX_UINT256, {
      from: lmsrOwner,
    });
  ...
  function shouldBeCreatedAndLiquid(numConditions, outcomesPerCondition) {
    ...
    context(
      `with ${numConditions} conditions and ${outcomesPerCondition} outcomes per condition`,
      function () {
        const conditions = [];
        it("preparing conditions", async function () {
          for (let i = 0; i < numConditions; i++) {
            ...
            conditions.push(
              getConditionId(oracle, questionId, outcomesPerCondition)
            );
          }
        ...
        it("creating the LMSR market maker", async function () {
          ...
          lmsrMarketMaker = await getParamFromTxEvent(
            tx,
            "lmsrMarketMaker",
            LMSRMarketMaker,
        ...
        it("approving LMSR for trading", async function () {
          await collateralToken.approve(lmsrMarketMaker.address, MAX_UINT256, {
            from: trader,
          });
